...
import torch
from submodules.EvalMuseMini.utils import get_index
from typing import Dict, Any

class ElementAlignmentMetric(torchmetrics.Metric):
    def __init__(self, model, vis_processors, text_processors, tokenizer, dataset_dir, **kwargs):
    ...
    def update(self, item: Dict[str, Any]):
        ...
        if "element_score" not in item:
            raise KeyError("item must contain 'element_score' key")
            
        # Handle case where element_score might be a string or improperly formatted
        element_scores = item["element_score"]
        ...
        with torch.no_grad():
            alignment_score, scores = self.model.element_score(image.unsqueeze(0), [processed_prompt])
        ...
        for element in elements:
            ...
            if idx:
                ...
                elements_score[element] = (scores * mask).sum().item() / mask.sum().item()
            else:
